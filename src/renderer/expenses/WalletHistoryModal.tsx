...
export function WalletHistoryModal({
  wallet,
  onClose,
}: WalletHistoryModalProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    void window.api.expenses
      .listWalletTransactions(wallet.id)
      .then((txns) => {
        if (!isActive) {
          return;
        }
      ...
      .catch(() => {
        ...
        addToast({ type: "error", message: "Failed to load wallet history." });
      })
      .finally(() => {
        if (isActive) {
          setLoading(false);
